...
- **Actieve fase** centraal (`stage`: `home` / `fase1` / `voting` (pauze 1 = dotvoten) / `fase2` / `pauze2` (pauze 2 = chillen) / `fase3` / `done`). Deelnemers kiezen niet meer zelf hun fase; de front-end toont automatisch de actieve fase. De lobby (`home`) toont de agenda; elke fase toont een "Deze ronde werkt als volgt" + "Na deze fase presenteer je"-blok.
